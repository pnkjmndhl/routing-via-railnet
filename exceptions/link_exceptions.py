...
link_exception_shp_dict = {
row.getValue("FID"): [row.getValue("dir"), row.getValue("RR") row.getValue("comm"), row.getValue("costptrhr"),
                      row.getValue("costptml"), row.getValue("NEAR_FID")] for row in
arcpy.SearchCursor(link_exception_shp)}
...
linkexc = linkexc[['ID', 'RR', 'comm', 'direction', 'costpertrainhr', 'costpertonmile']].apply(
